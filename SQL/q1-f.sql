SELECT COUNT(DISTINCT(c.customer_id)) cust_num FROM customer AS c INNER JOIN rental AS r ON r.customer_id = c.customer_id 
...
WHERE ca.name != "Horror" AND ca.name = 'Action' AND c.active = 1;